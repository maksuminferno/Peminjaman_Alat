...
@section('title', 'Daftar Alat - Sistem Peminjaman Alat')
...
<div class="container-fluid">
    ...
    @forelse($groupedAlat as $namaAlat => $tools)
        @php
        ...
        <div class="tool-card {{ $totalStock > 0 ? '' : 'unavailable' }}" data-nama-alat="{{ $namaAlat }}">
            <!-- Card Image -->
            <div class="tool-card-image">
                @if($totalStock > 0)
                <div class="tool-card-checkbox">
                    <input type="checkbox" class="tool-checkbox"
                        data-nama-alat="{{ $namaAlat }}"
                        data-tool-ids='@json($toolIds)'
                        data-stok="{{ $totalStock }}"
                        onchange="updateBorrowButton()">
                </div>
                @endif
                <i class="fas fa-tool-box"></i>
                <span class="tool-card-badge {{ $totalStock > 0 ? 'available' : 'unavailable' }}">
                    {{ $totalStock > 0 ? 'Tersedia' : 'Tidak Tersedia' }}
                </span>
            </div>

            <!-- Card Body -->
            <div class="tool-card-body">
                <h5 class="tool-card-title">{{ $namaAlat }}</h5>

                <div class="tool-card-info">
                    <div class="tool-card-info-item">
                        <i class="fas fa-tag"></i>
                        <span>{{ $firstTool->kategori->nama_kategori }}</span>
                    </div>
                    <div class="tool-card-info-item">
                ...
                <div class="tool-card-actions">
                    @if($totalStock > 0)
                        <a href="{{ route('peminjam.ajukan.peminjaman', ['id' => $firstTool->id_alat]) }}"
...
<script>
    ...
    function borrowSelected() {
        ...
        window.location.href = '{{ route("peminjam.ajukan.peminjaman") }}?alat=' + uniqueIds.join(',');
    }
